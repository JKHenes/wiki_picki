...
def Vtuber(website):
    top_list = get_url(website, "api/v1/Articles/Top")
    top_list = top_list.decode("utf-8")
    top_list = json.loads(top_list)
    best_girl =  random.choice(top_list.get("items")).get("id")
    best_girl_results = get_url(website, "api/v1/Articles/Details?ids=" + str(best_girl))
    best_girl_results= best_girl_results.decode("utf-8")
    best_girl_results= json.loads(best_girl_results)

    return best_girl_results, best_girl
...
def get_info(query,website):
    ...
    elif website == "virtualyoutuber.fandom.com":
        search_data, article_id = Vtuber(website)
        Vtuber_f = True
    ...
    if not Vtuber_f:
        ...
        article_data = search_data.get("items").get(str(article_id))
        title = article_data.get("title")
        url = "https://"+ website +article_data.get("url")
        text = article_data.get("abstract")
        image = article_data.get("thumbnail")[:-61]
